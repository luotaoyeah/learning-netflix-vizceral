...
class TrafficFlow extends React.Component {
  ...
  handlePopState() {
    ...
    this.setState({
      currentView: state.selected,
      objectToHighlight: state.highlighted
    });
  }
  ...
  viewChanged = data => {
    ...
    if (hasOwnPropFunc.call(data, "graph")) {
      ...
      if (oldCurrentGraph !== newCurrentGraph) {
        ...
        const o =
          newCurrentGraph === null ? null : newCurrentGraph.getPhysicsOptions();
  ...
  checkInitialRoute() {
    ...
    this.setState({
      ...
      objectToHighlight: parsedQuery.highlighted
    });
  }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      ...
      if (this.poppedState) {
        ...
        const highlightedObjectName =
          nextState.highlightedObject && nextState.highlightedObject.getName();
        const state = {
          title: document.title,
          url: `/${nextState.currentView.join("/")}${
            highlightedObjectName ? `?highlighted=${highlightedObjectName}` : ""
          }`,
  ...
  render() {
    ...
    const focusedNode =
      this.state.currentGraph && this.state.currentGraph.focusedNode;
    const nodeToShowDetails =
      focusedNode ||
      (this.state.highlightedObject &&
      this.state.highlightedObject.type === "node"
        ? this.state.highlightedObject
        : undefined);
    const connectionToShowDetails =
      this.state.highlightedObject &&
      this.state.highlightedObject.type === "connection"
        ? this.state.highlightedObject
        : undefined;
    const showLoadingCover = !this.state.currentGraph;
    const showBreadcrumbs =
      trafficData && trafficData.nodes && trafficData.nodes.length > 0;
      ...
      <div className="vizceral-container">
        {this.state.redirectedFrom ? (
          <Alert onDismiss={this.dismissAlert}>
            <strong>{this.state.redirectedFrom.join("/") || "/"}</strong> does
            not exist, you were redirected to{" "}
            <strong>{this.state.currentView.join("/") || "/"}</strong> instead
          </Alert>
        ) : (
          undefined
        )}
        <div className="subheader">
          {showBreadcrumbs && (
            <Breadcrumbs
              ...
              navigationStack={this.state.currentView || []}
              navigationCallback={this.navigationCallback}
            />
          )}
          {showBreadcrumbs && (
            <UpdateStatus
              status={this.state.regionUpdateStatus}
              baseOffset={this.state.timeOffset}
          ...
          <div style={{ float: "right", paddingTop: "4px" }}>
            {!focusedNode && matches && (
              <Locator
                changeCallback={this.locatorChanged}
                searchTerm={this.state.searchTerm}
                matches={matches}
                clearFilterCallback={this.filtersCleared}
              />
            )}
            <OptionsPanel title="Filters">
              <FilterControls />
            </OptionsPanel>
            <OptionsPanel title="Display">
              <DisplayOptions
                options={this.state.displayOptions}
                changedCallback={this.displayOptionsChanged}
              />
            </OptionsPanel>
            <OptionsPanel title="Physics">
              <PhysicsOptions
                options={this.state.currentGraph_physicsOptions}
                changedCallback={this.physicsOptionsChanged}
              />
            </OptionsPanel>
            <a
              role="button"
              className="reset-layout-link"
              onClick={this.resetLayoutButtonClicked}
            >
              Reset Layout
            </a>
          </div>
        ...
        <div className="service-traffic-map">
          <div
            style={{
              ...
              top: "0px",
              right:
                nodeToShowDetails || connectionToShowDetails ? "380px" : "0px",
              bottom: "0px",
              left: "0px"
            }}
          >
            <Vizceral
              ...
              allowDraggingOfNodes={
                this.state.displayOptions.allowDraggingOfNodes
              }
            />
          </div>
          {!!nodeToShowDetails && (
            <DetailsPanelNode
              node={nodeToShowDetails}
          ...
          {!!connectionToShowDetails && (
            <DetailsPanelConnection
              connection={connectionToShowDetails}
              region={this.state.currentView[0]}
              width={panelWidth}
              closeCallback={this.detailsClosed}
              nodeClicked={node => this.nodeClicked(node)}
