...
const nameRenderer = function(data) {
  ...
  const mostSevereNotice =
    data.rowData.notices &&
    data.rowData.notices.length > 0 &&
    _.maxBy(data.rowData.notices, notice => notice.severity);
  if (mostSevereNotice) {
  ...
  return (
    <span
      ...
      style={{ color: data.rowData.color }}
    >
      {data.cellData}
      {mostSevereNotice ? (
...
const errorRenderer = function(data) {
  return (
    <span
      className={data.rowData.className}
      data-error-rate={data.rowData.errorRate}
      style={{ color: data.rowData.color }}
...
class ConnectionList extends React.Component {
  ...
  render() {
      ...
      return {
        name:
          this.props.direction === "incoming"
            ? connection.source.getName()
            : connection.target.getName(),
    ...
    const tableHeight = Math.min(
      maxTableHeight,
      estimatedRowHeight * connectionRows.length + headerHeight
    );
