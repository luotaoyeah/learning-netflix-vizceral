...
const nameRenderer = function(data) {
  return (
    <span className={data.rowData.className} title={data.rowData.name}>
      {data.rowData.name}
    </span>
  );
};
...
const errorRenderer = function(data) {
  return (
    <span
      ...
      title={`${numeral(data.rowData.errors || 0).format("0.[00]")} / ${numeral(
        data.rowData.total || 0
      ).format("0.[00]")}`}
    >
      {numeral(data.rowData.errorRate || 0).format("0.[00]%")}
    </span>
  );
};
...
const totalRenderer = function(data) {
  return (
    <span
      className={data.rowData.className}
      title={numeral(data.rowData.total || 0).format("0.[00]")}
    >
      {numeral(data.rowData.totalPercent || 0).format("0.[00]%")}
    </span>
  );
};
...
class SubNodeList extends React.Component {
  constructor(props) {
    ...
    this.linkPopover = node => {
      const atlasBackend = this.props.region
        ? `&backend=http:%2F%2Fatlas-main.${
            this.props.region
          }.prod.netflix.net:7001`
        : "";
      const atlasLink = `http://atlasui.prod.netflix.net/ui/graph?g.q=nf.cluster,${
        node.name
      },:eq,name,RequestStats-all-requests-_Num,:re,:and,:sum,(,name,),:by&g.e=now-1m&g.s=e-3h&g.tz=US%2FPacific&mode=png&vsplit=520px&sel=expr.0.0${atlasBackend}`;
      const spinnakerRegion = this.props.region
        ? `&reg=${this.props.region}`
        : "";
      const spinnakerLink = `https://spinnaker.prod.netflix.net/#/applications/${
        node.app
      }/clusters?acct=prod&q=cluster:${node.name}${spinnakerRegion}`;
      return (
        <Popover id="links-popover">
          <div className="links-popover-title">OPEN IN</div>
          <div className="links-popover-link">
            <a href={atlasLink} target="_blank">
              Atlas UI
            </a>
          </div>
          <div className="links-popover-link">
            <a href={spinnakerLink} target="_blank">
              Spinnaker
            </a>
          </div>
        </Popover>
    ...
    this.linkRenderer = data =>
      data.rowData.app ? (
        <OverlayTrigger
          ...
          rootClose
          placement="bottom"
          overlay={this.linkPopover({
            app: data.rowData.app,
  ...
  render() {
    ...
    const trafficTotal = _.sum(
      _.flatten(_.map(this.state.nodes, c => _.values(c.metrics)))
    );
    ...
    const tableHeight = Math.min(
      maxTableHeight,
      estimatedRowHeight * nodeRows.length + headerHeight
    );
